...
Uma biblioteca de tabela React poderosa e personalizável, construída com TypeScript e TanStack Table.
...
interface SelectableProps {
    ...
    onSelectedRowsChange?: (                    // Callback de mudança
        ids: (string | number)[]
    ) => void;
}
```
...
interface PaginationProps {
    ...
    onPageChange: (                 // Callback de mudança
        page: number,
        pageSize: number
    ) => void; 
}
...
interface ColumnMeta<TData, TValue> {
    widthSize?: string;                                 // Largura da coluna
